...
- **`AGENTS.md`**：入口摘要，只保留最短执行顺序、角色边界、CLI 入口与读产物顺序。
- **`AI_股票对话提示.md`（本文件）**：唯一完整规范来源；所有公式、例外、扩展说明与章节号统一以本文件为准。
- **`.cursor/rules/stock-analysis-agent.mdc`**：Cursor 常驻硬规则，只保留必须始终生效的约束；若其表述比本文件更严，执行更严者。

维护原则：新增细则优先写入本文件；`AGENTS.md` 只同步摘要；`.mdc` 只补必须常驻的最小规则，不复制整段操作说明。
