#!/usr/bin/env python3
"""
真实调用 DeepSeek「飞书意图路由」API（与线上 decide_feishu_route 使用同一套请求体）。
...
from tools.deepseek.client import DeepSeekError, feishu_route_deepseek_raw_and_routed  # noqa: E402


def main() -> int:
    p = argparse.ArgumentParser(description="真实调用 DeepSeek 飞书路由（非单元测试 mock）。")
    ...
    try:
        ...
        print("DeepSeekError:", exc, file=sys.stderr)
        return 1

    print("=== DeepSeek chat/completions 原始响应（完整 JSON）===")
